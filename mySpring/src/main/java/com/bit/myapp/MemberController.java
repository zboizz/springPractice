...
public class MemberController {
	...
	public String join(@RequestParam("id")String id,
		...
		MemberDto dto = new MemberDto(); //dto선언
		//dto에 id, pwd, name, email 데이터 집어넣기
		dto.setId(id);
		dto.setPwd(pwd);
		dto.setName(name);
		dto.setEmail(email);
		model.addAttribute("member",dto);//member라는 공간에 dto의 데이터를 넣겠다.
		
		return "member/join"; //member/join.jsp
	}
	@RequestMapping(value="member/join1")
	//1. JSP에서 MemberDto를 가져온다.(자동적으로 setter 호출값이 저장)
